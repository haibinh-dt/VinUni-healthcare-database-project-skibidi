...
--    -> just comment out the duplicates or create them in create_tables.sql.
-- -----------------------------------------------------------

-- Appointment: FK helper + doctor schedule
-- (You already have idx_appointment_patient and idx_appointment_doctor_date in CREATE TABLE.)
-- Keep them. Add patient+date composite for patient history queries:
-- If idx_appointment_patient_date already exists, comment this out.
...
-- CREATE INDEX idx_availability_doc_date ON Doctor_Availability(doctor_id, available_date);
